...
int main(int argc, char** argv)
{
	...
	while (node.ok())
	{
		...
		try
		{
			...
			listener.lookupTransform("/tb3_3", "/tb3_2", ros::Time(0), transformfl);
		...
		try
		{
		        listener.waitForTransform("/tb3_2", "/tb3_3", ros::Time(0), ros::Duration(3.0));
			listener.lookupTransform("/tb3_2", "/tb3_3", ros::Time(0), transformlf);
		...
		// 根据tb3_1与tb3_2坐标系之间的位置关系，发布tb3_2的速度控制指令
